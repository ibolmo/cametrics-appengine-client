...
class Cametrics:
  ...
  def _post_data(self):
    ...
    cmurl = Cametrics.getURL()
    logging.debug('Cametrics posting: %s' % cmurl)
    ...
    result = urlfetch.fetch(url=cmurl, payload=data, method=urlfetch.POST, headers={'Content-Type': 'application/x-www-form-urlencoded'}, deadline=10)
  ...
  def commit():
    ...
    #print Cametrics.getInstance().data

  def post(self, namespace, value, vtype):
        #print namespace, value, vtype
        self.data.append({
          'namespace': namespace,
          'value': value,
          'type': vtype
